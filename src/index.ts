...
import createEvents from "./hooks/events/createEvents"
...
export class Main {
  ...
  async initCommands() {
    const files = glob(path.join(__dirname, "discord", "commands", "*.{ts,js}"))
  ...
  async initDiscordEvents() {
    await createEvents({
      paths: path.join(__dirname, "discord", "events", "*.ts"),
      ctx: {
        client: this.client,
        commands: this.commands,
        db: this.db,
